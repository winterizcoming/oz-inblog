...
├─ releases/1.0.0-alpha.7/
├─ current -> releases/1.0.0-alpha.7/
├─ data/
├─ config/
└─ backups/
```
...
5. GitHub Release `v1.0.0-alpha.7`의 `oz-inblog-1.0.0-alpha.7.tar.gz`와 `.sha256`을 같은 임시 폴더에 다운로드합니다.
...
앱을 다시 실행하면 공개 릴리스 목록에서 최신 alpha·beta·stable 버전을 확인합니다. 새 버전이 있으면 업데이트용 Codex 안내를 클립보드에 복사할지 묻습니다. 실행 중인 앱은 항상 `current` 릴리스의 서버를 사용하며, `/Applications/oz-inblog.app`과 `~/Applications/oz-inblog.app`이 동시에 있으면 하나를 현재 릴리스에 동기화하고 나머지는 `backups/apps`로 옮깁니다. 데이터와 설정은 release 폴더 밖에 있으므로 업데이트해도 보존됩니다.
